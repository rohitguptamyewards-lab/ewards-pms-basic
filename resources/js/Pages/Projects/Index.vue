                            ...
                            <td class="px-4 py-3 overflow-hidden">
                                ...
                                >
                                    <div v-if="p.last_comment" class="flex items-start gap-1 relative group/tooltip">
                                        ...
                                        <div>
                                            <p class="text-xs text-gray-600 group-hover/comment:text-[#4e1a77] whitespace-nowrap">{{ truncateWords(p.last_comment) }}</p>
                                            <p class="text-[10px] text-gray-400">{{ timeAgo(p.last_comment_at) }}</p>
                                        </div>
                                        <!-- Tooltip -->
                                        <div v-if="p.last_comment.trim().split(/\s+/).length > 6" class="pointer-events-none absolute bottom-full left-0 mb-1.5 z-50 hidden group-hover/tooltip:block">
                                            <div class="rounded-lg bg-gray-900 px-3 py-2 text-xs text-white shadow-lg max-w-[240px] whitespace-pre-wrap break-words">
                                                {{ p.last_comment }}
                                                <div class="absolute top-full left-4 -mt-px border-4 border-transparent border-t-gray-900"></div>
                                            </div>
                                        </div>
                                    </div>
